...
function updateBrightnessUi(v, skipValueSet) {
    const value = clamp(parseInt(v, 10) || 0, 0, 100);
    const slider = el('dp-brt');
    if (slider) {
        // Re-assigning .value while the user is mid-drag on this same slider
        // (skipValueSet) can make the browser snap/jump the thumb; only force
        // the value when syncing from an external source (e.g. device state).
        if (!skipValueSet) slider.value = String(value);
...
function handleWsMessage(d) {
    ...
    if (d.settings) {
        ...
        if (!_db.disp) {
            ...
            if (s.bg_effect_en != null && !isFocused('dp-anim-effect')) {
...
function init() {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (drop.contains(e.target) || e.target.closest('#mnav-btn')) return;
        closeMobileNav();
    });
